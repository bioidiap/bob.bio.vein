...
class ManualRoi(Preprocessor):
    """ManualRoi preprocessor class - preprocessor that returns both - original
       ...
       needs to be used with entry-point ``biowave_v1_e`` (that returns
       ``dict`` object insetead of image).
    """
    ...
    def __call__(self, annotation_dictionary, annotations = None):
        """
          ...
          Args:
            annotation_dictionary (:py:class:`dict`): Dictionary containing
            image and annotation data. Such :py:class:`dict` can be returned
            by the high level ``bob.db.biowave_v1``  implementation of the
            ``bob.db.biowave_v1`` database. It is supposed to contain fields
            (as can be returned by the ``bob.db.biowave_v1`` high level
            implementation using ``setup`` entry point ``biowave_v1_e``):
    ...
    def write_data(self, data, file_name):
        ...
        function of this class) to file. This method overwrites the
        write_data() method of the Preprocessor class.
