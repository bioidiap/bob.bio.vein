...
#
#        enroll_dilated = ndimage.morphology.binary_dilation(enroll, structure = ellipse_kernel).astype(np.uint8)
#
#        intersection = enroll_dilated * probe
#
#        normalizer = np.sum(enroll) + np.sum(probe)
#
#        score = np.sum(intersection) / normalizer

#ellipse_kernel = morphology.disk(radius = kernel_radius)
#
#enroll_dilated = ndimage.morphology.binary_dilation(enroll, structure = ellipse_kernel).astype(np.uint8)
#
#probe_dilated = ndimage.morphology.binary_dilation(probe, structure = ellipse_kernel).astype(np.uint8)
#
#intersection_1 = enroll_dilated * probe
#
#intersection_2 = probe_dilated * enroll
#
#enroll_or_intersection_1 = np.bitwise_or( enroll.astype(np.uint8), intersection_1.astype(np.uint8) ).astype(np.float)
#
#probe_or_intersection_2 = np.bitwise_or( probe.astype(np.uint8), intersection_2.astype(np.uint8) ).astype(np.float)
#
#normalizer = np.sum(enroll_or_intersection_1) + np.sum(probe_or_intersection_2)
#
#score = np.sum( enroll_or_intersection_1 * probe_or_intersection_2 ) / normalizer
