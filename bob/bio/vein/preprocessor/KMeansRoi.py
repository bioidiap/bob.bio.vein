...
class KMeansRoi(Preprocessor):
    ...
    def __init__(self, filter_name = "median_filter", mask_size = 7,
        ...
        **Parameters:**
        ...
        ``fixed_scaling_flag`` : :py:class:`bool`
            Rescale the ROI and the image if True, given fixed scaling factor. Default value: False.

        ``scale`` : :py:class:`float`
            Scaling factor for the ROI and input image. Only valid when
            ``fixed_scaling_flag`` is set to ``True``. Default value: 1.
        ...
        Preprocessor.__init__(self,
                                ...
                                fixed_scaling_flag = fixed_scaling_flag,
                                scale = scale,
        ...
        self.fixed_scaling_flag = fixed_scaling_flag
        self.scale = scale
...
#        mask_scaled[mask_scaled>0.1] = 1
    ...
    #==========================================================================
    def rescale(self, image, mask, scale):
        """
        This function normalizes the scale of the ROI and of the input image.
        The parameter ``mask_to_image_area_ratio`` defines the area of the
        normalized ROI, as a fraction of the area of the input image.
        ...
        **Parameters:**
        ...
        ``scale`` : :py:class:`float`
            Scaling factor for the ROI and input image.
        ...
        **Returns:**
        ...
        tform = tf.SimilarityTransform(scale = 1./scale, rotation = 0, translation = (0, 0))
...
#        mask_scaled[mask_scaled>0.1] = 1
    ...
    def __call__(self, image, annotations):
        ...
        **Returns:**
        ...
        if self.normalize_scale_flag: # the scaling factor is tuned automatically for each image

            image_transformed, mask_binary_transformed = self.normalize_scale(image,
                                                                              mask_binary,
                                                                              self.mask_to_image_area_ratio)

        elif self.fixed_scaling_flag: # the scaling factor is fixed for all images

            image_transformed, mask_binary_transformed = self.rescale(image,
                                                                      mask_binary,
                                                                      self.scale)
