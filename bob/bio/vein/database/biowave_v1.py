...
class BiowaveV1BioFile(VeinBioFile):
    def __init__(self, low_level_file, client_id, path, file_id, protocol, extra_annotation_information):
        """
...
class BiowaveV1BioDatabase(BioDatabase):
    ...
    **Configurations / Entry points**
    ...
    def model_ids_with_protocol(self, groups=None, protocol=None, **kwargs):
        return self.__db.model_ids(protocol=protocol, groups=groups, annotated_images=self.annotated_images, imagedir = self.original_directory)

    def objects(self, protocol=None, groups=None, purposes=None, model_ids=None, **kwargs):
        retval = self.__db.objects(protocol=protocol, groups=groups, purposes=purposes, model_ids=model_ids, sessions=None, attempts=None, im_numbers=None, annotated_images=self.annotated_images, imagedir = self.original_directory)
        return [BiowaveV1BioFile(f, client_id=f.client_id, path=f.path, file_id=f.id, protocol=protocol, extra_annotation_information = self.extra_annotation_information) for f in retval]
